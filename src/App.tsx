...
const generateHex = (bytes = 32) => {
  ...
  return Array.from({ length: bytes * 2 }, () => chars[Math.floor(Math.random() * chars.length)]).join('');
};
  ...
  const renderDatabaseUrl = (url: string, pwd?: string) => {
    ...
    if (parts.length === 2) {
      return (
        <>
          {parts[0]}
          <span className="text-white bg-indigo-600/30 px-1 rounded">{pwd}</span>
          {parts[1]}
        </>
      );
    }
      ...
      <div className="max-w-4xl w-full flex flex-col gap-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 border-b border-slate-800 pb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white tracking-tight">Talos Env Generator</h1>
              <p className="text-slate-500 text-sm">Gere credenciais seguras para seu ambiente local</p>
            </div>
          </div>
          <button
        ...
        {keys.POSTGRES_PASSWORD && (
          <div className="flex-grow grid grid-cols-1 gap-6">
              ...
              <div className="space-y-6 mt-12 sm:mt-8">
                {[
                  ...
                  <div key={item.label} className="group relative">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-bold text-slate-500 tracking-widest uppercase">
                        ...
                        {item.label === 'DATABASE_URL' && (
                          <span className="ml-2 text-emerald-500 italic lowercase font-normal hidden sm:inline">(senha injetada automaticamente)</span>
                        )}
                      </span>
                      <button
                        onClick={() => copyToClipboard(`${item.label}=${item.value}`, item.label)}
                    ...
                    <div className={`border rounded-xl p-4 font-mono break-all text-sm sm:text-base ${
                      item.label === 'DATABASE_URL'
                      ...
                      {item.label === 'DATABASE_URL' ? renderDatabaseUrl(item.value || '', keys.POSTGRES_PASSWORD) : item.value}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-2 text-center">
              <p className="text-slate-600 text-xs italic">As chaves geradas são únicas e nunca armazenadas em nossos servidores.</p>
            </div>
          </div>
        )}
      </div>
